...
class OneDollar(object):
    ...
    def recognize(self, points):
        ...
        for i in range(len(self.templates)):
            d = self.distanceAtBestAngle(newPoints, self.templates[i], -self.angle_range, self.angle_range, self.angle_step)
            if d < b:
                b = d
                template_id = i
                label = self.labels[i]
    ...
    def rotateToZero(self, points):
        ...
        theta = np.arctan2(centroid[0] - points[0][0], centroid[1] - points[0][1])
        
        if theta <= np.pi and theta >= -np.pi:
            return self.rotateBy(points, theta)
        return points # Si le theta ne correspond pas
    ...
    def rotateBy(self, points, angle):
        ...
        for p in points:
            ...
            newPoints=np.append(newPoints,[q],axis=0)
    ...
    def scaleToSquare(self, points):
        ...
        coefx = self.square_size / (max(points[:,0])-min(points[:,0]))
        coefy = self.square_size / (max(points[:,1])-min(points[:,1]))
        #todo 7
        for p in points:
            q = np.array([0, 0])
            q[0] = p[0] * coefx
